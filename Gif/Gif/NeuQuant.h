...
class CNeuQuant 
{
	...
	pic = (unsigned byte*) malloc(3*width*height);
	...
	static const int betagamma =
		(intbias << (gammashift - betashift));

	/* defs for decreasing radius factor */
	static const int initrad = (NETSIZE >> 3); /* for 256 cols, radius starts */
	static const int radiusbiasshift = 6; /* at 32.0 biased by 6 bits */
	static const int radiusbias = (((int) 1) << radiusbiasshift);
	static const int initradius = (initrad * radiusbias); /* and decreases by a */
	...
	int network[NETSIZE][4]; /* the network itself - [NETSIZE][4] */
...
public:
	...
	----------------------------------------------------------------------- */
	CNeuQuant(byte* thepic, int len, int sample) 
		:thepicture(thepic),
		lengthcount(len),
		samplefac(sample)
	{
